	...
	*/
	...
	private function create_command_for_button_callback($command) {
		...
		if (stripos($buttonCallback, 'allForums') === 0 ) {
			...
			unset($command['message_id']); //No update of telegram message, but request answer below of previous post
	...
	private function send_admin_info($input, $command) {
		...
		if (!isset($command['admin_info'])) return;
	...
	private function onAllForumTopics($user_id, $chat_id, $forum_id, $page = 0, $warning = false) {
		...
		if ($forum) {
			...
			foreach($topics as $id => $topic) {
				...
				$i++;
				if ($i > 6) break; //Not more than 8 buttons allowed.
			}
			if ($warning) {
				// 
	...
	private function onAllForums($user_id, $chat_id, $back_to_forum_id, $page = 0, $warning = false) {
		...
		foreach($forums as $forum) {
			...
			$i++;
			if ($i > 6) break; //6 per page, such that the button list stays short.
		}
		if ($warning) {
